<template>
    ...
    backgroundColor:datas.setting ? datas.setting.columnBgColor : '#FFF'}">
        ...
        <div class="WidgetGoods-goodsListX3 Widget-skeleton" v-if="(!datas || !datas.goods || datas.goods.length <= 0)">
        ...
        <div class="WidgetGoods-goodsListX3" v-if="(datas && datas.goods && datas.goods.length > 0)">
...
<script>
    export default {
        ...
        visible: true,
        category: 'Goods',
        preview: require('@/assets/images/designer-widget/preview-GoodsX3.png'),
        fields: [
            {
                type:'goods',
                config: {
