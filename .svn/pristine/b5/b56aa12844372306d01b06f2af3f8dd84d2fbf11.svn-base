<template>
    <div class="designer-uploader-container">
        <div class="uploader-header">
            <a-alert :message="'当前组件最大子项('+maxNum+'个)已满'" banner v-if="dataList.length >= maxNum"/>
            <a-button style="width: 100%" @click="onModalShow"
                      v-if="dataList.length < maxNum">
                <a-icon type="plus"/>
                添加数据
            </a-button>
        </div>
        <div class="uploader-body">
            <div class="data-list">
                <a-row :gutter="5">
                    <a-col :span="24 / colNum" v-for="(item,index) in mineData" :key="index" :style="{marginBottom: '5px'}">
    ...
    export default {
        ...
        computed:{
            mineData:function () {
                ...
                if(self.datas.upload && self.datas.upload.length > 0){
                    self.dataList = self.datas.upload;
                }
