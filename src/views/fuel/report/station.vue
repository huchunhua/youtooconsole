    ...
    <div>
        ...
        <div class="margin-bottom">
            ...
            <a-select :defaultValue="type[0].value" style="width: 120px" @change="handleTypeChange">
              <a-select-option v-for="type in type" :value="type.value" :key="type.value">{{type.name}}</a-select-option>
            </a-select>
    ...
    export default {
        ...
        methods: {
            ...
            Download(type) {
              ...
              window.open('http://sichuan.95504.net/v4/console/bill/countReportStation?' + 'token=' + token +'&time_type=' + time_type + '&time_start=' + time_start + '&time_end=' + time_end + '&station_code=' + this.id + '&order_type=' + order_type + '&is_export=1');
            },
            onChange(current) {
              const _this = this;
              const time_type = _this.time_type;
              ...
              this.$model.bill.getStationList({time_type:time_type,time_start:time_start,time_end:time_end,station_code:this.id,order_type:order_type,page:current}).then((response) => {
